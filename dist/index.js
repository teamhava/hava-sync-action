...
const http_client_1 = __nccwpck_require2_(59);
const auth_1 = __nccwpck_require2_(402);
...
const pm = __nccwpck_require2_(437);
let tunnel;
var HttpCodes;
(function (HttpCodes) {
...
const http = __importStar(__nccwpck_require2_(605));
const https = __importStar(__nccwpck_require2_(211));
...
var util = __nccwpck_require2_(669);
...
const http = __nccwpck_require2_(255);
...
async function run() {
  try {
    ...
    const viewId = core.getInput('view_id');
    const havaToken = core.getInput('hava_token');
    const imageName = core.getInput('image_name');

    const headers = {'Authorization': `Bearer ${havaToken}`};

    const client = new http.HttpClient();
    const result = await client.post(`https://api.hava.io/sources/${sourceId}/sync`, '{}', headers);

    if (result.message.statusCode != 200) {
      var message = `Sync request failed with http status code: ${result.message.statusCode}`;
      console.log(message);
      throw new Error(message);
    }

    // const job = await result.readBody();
